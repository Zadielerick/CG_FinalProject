...
int main(int argc, char* argv[])
{
    ...
    if( s.inputType == Settings::IMAGE_LIST && s.showUndistorted && !cameraMatrix.empty())
    {
        ...
        std::vector<cv::String> orig_images;
        std::string orig_path = "pictures/*SRV*.jpg";

        cv::glob(orig_path,orig_images);

        for(size_t i = 0; i < orig_images.size(); i++ )
        {
            view = imread(orig_images[i], IMREAD_COLOR);
            if(view.empty())
                continue;
            remap(view, rview, map1, map2, INTER_LINEAR);
            imshow("Image View", rview);
            ...
            std::string out_file_name = int_to_str(i) + "srv_out.png";
            std::cout << out_file_name << std::endl;
            imwrite(out_file_name,rview,compression);
            char c = (char)waitKey();
            if( c  == ESC_KEY || c == 'q' || c == 'Q' )
                break;
        }
